...
{
    /// <summary>获取或初始化输入流最大字节数，默认 128 MiB；显式设置 null 可关闭该限制。</summary>
    public long? MaxInputBytes { get; init; } = 128L * 1024 * 1024;

    /// <summary>获取或初始化最大数据行数；null 表示不额外限制。</summary>
    public int? MaxRows { get; init; }

    /// <summary>获取或初始化最大错误数；null 表示不额外限制。</summary>
    public int? MaxErrors { get; init; }

    /// <summary>获取或初始化最大图片数量；null 表示不额外限制。</summary>
    public int? MaxPictures { get; init; }

    /// <summary>获取或初始化单张图片最大字节数；null 表示不额外限制。</summary>
    public long? MaxPictureBytes { get; init; }

    /// <summary>获取或初始化所有图片最大总字节数；null 表示不额外限制。</summary>
    public long? MaxTotalPictureBytes { get; init; }

    /// <summary>获取或初始化单个工作表最大跟踪唯一值数量；null 表示不额外限制。</summary>
    public int? MaxTrackedUniqueValues { get; init; }

    /// <summary>获取或初始化唯一值比较策略；默认使用忽略大小写的序号比较。</summary>
    public StringComparison UniqueComparison { get; init; } = StringComparison.OrdinalIgnoreCase;

    /// <summary>获取或初始化XLSX ZIP 最大 entry 数量；为空表示不额外限制。</summary>
    public int? MaxZipEntries { get; init; } = 10000;

    /// <summary>获取或初始化XLSX ZIP 单个 entry 最大解压字节数；为空表示不额外限制。</summary>
    public long? MaxZipEntryUncompressedBytes { get; init; } = 64L * 1024 * 1024;

    /// <summary>获取或初始化XLSX ZIP 所有 entry 最大总解压字节数；为空表示不额外限制。</summary>
    public long? MaxZipTotalUncompressedBytes { get; init; } = 512L * 1024 * 1024;

    /// <summary>获取或初始化XLSX XML 节点和值的最大字符数量；为空表示不额外限制。</summary>
    public long? MaxXmlCharacters { get; init; } = 64L * 1024 * 1024;

    /// <summary>获取或初始化XLSX XML 最大嵌套深度；为空表示不额外限制。</summary>
    public int? MaxXmlDepth { get; init; } = 256;

    /// <summary>获取或初始化XLSX ZIP 允许的最大解压/压缩比；为空表示不额外限制。</summary>
    public double? MaxZipCompressionRatio { get; init; } = 1000;

    /// <summary>获取或初始化sharedStrings.xml 最大解压字节数；为空表示不额外限制。</summary>
    public long? MaxSharedStringsBytes { get; init; } = 64L * 1024 * 1024;

    /// <summary>获取或初始化styles.xml 最大解压字节数；为空表示不额外限制。</summary>
    public long? MaxStylesBytes { get; init; } = 16L * 1024 * 1024;

    /// <summary>获取或初始化单个 worksheet XML 最大解压字节数；为空表示不额外限制。</summary>
    public long? MaxWorksheetBytes { get; init; } = 64L * 1024 * 1024;

    /// <summary>获取或初始化所有 worksheet XML 最大解压字节数；为空表示不额外限制。</summary>
