...
{
    /// <summary>获取或初始化失败工作簿模式。</summary>
    public ExcelImportFailureWorkbookMode Mode { get; init; }

    /// <summary>获取或初始化失败工作簿目标流，由调用方拥有。</summary>
    public Stream Destination { get; init; }

    /// <summary>获取或初始化失败工作簿序列化输出允许的最大字节数。</summary>
    public long? MaxSerializedBytes { get; init; }

    /// <summary>获取或初始化ErrorRowsOnly 模式允许复制的最大错误数据行数。</summary>
    public int? MaxCandidateErrorRows { get; init; }

    /// <summary>获取或初始化ErrorRowsOnly 模式允许复制的最大单元格估算数量。</summary>
    public long? MaxCopiedCells { get; init; }

    /// <summary>获取或初始化ErrorRowsOnly 模式允许复制的最大图片数量。</summary>
    public int? MaxCopiedPictures { get; init; }

    /// <summary>获取或初始化ErrorRowsOnly 模式允许复制的图片数据最大字节数。</summary>
    public long? MaxCopiedPictureBytes { get; init; }

    /// <summary>获取或初始化ErrorRowsOnly 模式允许目标工作簿估算对象的最大数量；估算包括 Sheet、行、单元格和图片对象。</summary>
    public long? MaxEstimatedTargetObjects { get; init; }

    /// <summary>获取或初始化失败工作簿请求级临时目录；为空时使用系统临时目录。</summary>
    public string TemporaryDirectory { get; init; }

    /// <summary>获取或初始化失败工作簿边界诊断接收器。</summary>
    public Action<ExcelImportFailureDiagnostic> DiagnosticSink { get; init; }

    /// <summary>获取或初始化AnnotatedOriginal 模式下的批注冲突策略，默认追加失败信息。</summary>
