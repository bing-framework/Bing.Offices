...
{
    /// <summary>
    /// 记录当前 staging 会话使用的内存、临时文件或混合策略。
    /// </summary>
    private readonly NpoiAsyncStagingStrategy _strategy;
    /// <summary>
    /// 记录混合策略从内存迁移到临时文件的字节阈值。
    /// </summary>
    ...
    /// <summary>
    /// 初始化一个 <see cref="NpoiAsyncStaging" /> 类型的实例。
    /// </summary>
    /// <param name="strategy">内存、临时文件或混合 staging 策略。</param>
    /// <param name="hybridThresholdBytes">混合策略迁移到临时文件的字节阈值。</param>
    /// <param name="prefix">临时文件名使用的前缀。</param>
    ...
    /// <summary>
    /// 提供带阈值迁移的 staging 写入流。
    /// </summary>
    /// <remarks>
    /// 写入内容超过混合策略阈值时迁移到临时文件。
    /// </remarks>
    private sealed class HybridThresholdWriteStream : Stream
    {
        /// <summary>
        /// 保存所属 staging 会话，用于转发写入并执行阈值迁移。
        /// </summary>
        private readonly NpoiAsyncStaging _owner;

        /// <summary>
        /// 初始化一个 <see cref="HybridThresholdWriteStream" /> 类型的实例。
        /// </summary>
